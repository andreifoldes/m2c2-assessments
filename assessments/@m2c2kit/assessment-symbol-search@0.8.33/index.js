...
if (webcamEnabled) {
  try {
    ...
    console.warn("[Symbol Search] Could not load webcam module:", e);
  }
}
...
if (lightEnabled) {
  try {
    ...
    if (ambientLightModule.isAmbientLightSupported()) {
      ...
      console.warn("[Symbol Search] AmbientLightSensor not supported by this browser.");
      ambientLightModule = null;
    }
  } catch (e) {
    console.warn("[Symbol Search] Could not load ambient light module:", e);
  }
}
...
session.onEnd(async () => {
  ...
  if (ambientLightModule) {
    try {
      await ambientLightModule.stopAndExportLight("symbol-search");
    } catch (e) {
      console.warn("[Symbol Search] Light export failed:", e);
    }
  }
...
if (webcamEnabled && webcamModule) {
  ...
  if (accepted) {
    try {
      ...
      console.warn("[Symbol Search] Webcam recording unavailable, proceeding without it.");
    }
  }
}
...
if (lightEnabled && ambientLightModule) {
  ...
  if (accepted) {
    try {
      ...
      console.warn("[Symbol Search] Ambient light sensing unavailable, proceeding without it.", e);
      ambientLightModule = null;
    }
  } else {
    ambientLightModule = null;
